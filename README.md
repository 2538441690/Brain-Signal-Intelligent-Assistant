1.项目描述
    ...
    https://poser-detached-napped.ngrok-free.dev。
    本项目最终提供了一个功能完整、可演示的智能助手，既可作为脑科学研究人员的问答工具，也可作为 Agent 技术的学习范例。代码结构清晰，注释齐全，支持横向扩展。

2.代码部署
(1) 准备运行环境
    python版本：3.10 或更高。
    安装 MySQL：创建数据库（agent_db），并创建一个用户（agent_user）。
    安装依赖：在项目根目录执行  
